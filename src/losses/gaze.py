#from https://github.com/neeek2303/EMOPortraits
import torch
...
class GazeLossRTGene(nn.Module):
    def __init__(self,
        ...
        checkpoints_paths_dict = {'vgg16':f'{project_dir}/src/losses/gaze_models/vgg_16_2_forward_sum.pt', 'resnet18':f'{project_dir}/src/losses/gaze_models/resnet_18_2_forward_sum.pt'}
        if interpolate:
            checkpoints_paths_dict = {'vgg16':f'{project_dir}/src/losses/gaze_models/vgg_16_2_forward_sum_inter.pt', 'resnet18':f'{project_dir}/src/losses/gaze_models/resnet_18_2_forward_sum_inter.pt'}  
    ...
    def forward(self,
        ...
        if keypoints is not None:
            keypoints_np = [kp[:, :2].cpu().numpy() / 512 * tgt.shape[2] for kp, tgt in zip(keypoints, target)]
